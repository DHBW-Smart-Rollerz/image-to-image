...
CONTROLNET_MODEL_LINEART = "lllyasviel/sd-controlnet-scribble"
LORA_PATH = "output_lora/sim2real_dashcam.safetensors"

CANNY_CONDITIONING_SCALE = 0.9
LINEART_CONDITIONING_SCALE = 0.7
...
controlnet_lineart = ControlNetModel.from_pretrained(
    CONTROLNET_MODEL_LINEART,
    torch_dtype=torch.float16,
)

pipe = StableDiffusionControlNetInpaintPipeline.from_pretrained(
    BASE_MODEL,
    controlnet=[controlnet_canny, controlnet_lineart],
    torch_dtype=torch.float16,
    safety_checker=None,
)
...
def sim2real(
    ...
    control_image_lineart = compute_canny(init_image)
    ...
    result = pipe(
        ...
        strength=0.85,
        guidance_scale=6.5,
        controlnet_conditioning_scale=[CANNY_CONDITIONING_SCALE, LINEART_CONDITIONING_SCALE],
    ...
    # Speichere auch die Control Maps
    base_path = os.path.splitext(out_path)[0]
    canny_path = f"{base_path}_canny.png"
    lineart_path = f"{base_path}_lineart.png"
    
    control_image_canny.save(canny_path)
    print(f"✓ Canny-Map gespeichert: {canny_path}")
    
    control_image_lineart.save(lineart_path)
    print(f"✓ FineLine-Map gespeichert: {lineart_path}")
